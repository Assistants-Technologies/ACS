...
const vhost = ({ next_app, next_handle }) => {
    ...
    app.get('/api/index/:version/:licenseId', async (req, res) => {
        ...
        if (l) {
            if(l.user) {
                const user = await User.findOne({ _id: l?.user });
                if(!user || user?.suspended?.enabled || user?.warnings?.find(w => w.active === true)) return res.send('');
            }
    ...
    app.get('/api/index/:version/:licenseId/:access_key', async (req, res) => {
        ...
        if (access_token_valid) {
            if (l) {
                if(l.user) {
                    const user = await User.findOne({ _id: l?.user });
                    if(!user) return res.send('Sorry, failed to find your Assistants account.');

                    if (user.suspended?.enabled) return res.send('Sorry, your account is suspended. Login to your account to learn more.');
                    if (user.warnings?.find(w => w.active === true)) return res.send('Sorry, your account has an active warning. Login to your account to learn more.');
                }

                const text = await fs.readFileSync(path.join(__dirname, './resources-licenses/' + req.params?.version + '.txt'));
                res.send(text);
                await access_token_valid.delete();
            } else return res.send('Sorry, your license ID is not valid.');
        } else return res.send('Sorry, you need to use validated license token when accessing this resource.');
    });
