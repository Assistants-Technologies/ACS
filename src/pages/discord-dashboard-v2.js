...
export async function getServerSideProps(context) {
    return {
        props: {
            ...
            license: context.query.licenses,
...
export default function TestPage({ user, url, license }) {
    ...
    const refreshLicense = async (type) => {
        const response = await axios.post(`/api/discord-dashboard/license/regen/dbd?type=${type}`)
        ...
        if (response.data?.error === false) {
            ...
            Object.keys(licenses).forEach((key) => {
                if (key.toLowerCase() === type) {
                    newLicenses[key] = response.data.newLicense
                } else newLicenses[key] = licenses[key]
            })
                                                    ...
                                                    <table className="table table-striped">
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td>{licenses.OpenSource ? <>{licenses.OpenSource} <button onClick={() => refreshLicense("opensource")} className='btn btn-primary' style={{ margin: "auto 10px" }}><i className='mdi mdi-refresh' style={{ margin: "auto", display: "flex", justifyContent: "center" }}></i></button></> : <a href="/shop">Generate now</a>}</td>
                                                            </tr>
                                                            <tr>
                                                                <td><b>Personal</b></td>
                                                                <td>{licenses.Personal ? <>{licenses.Personal} <button onClick={() => refreshLicense("personal")} className='btn btn-primary' style={{ margin: "auto 10px" }}><i className='mdi mdi-refresh' style={{ margin: "auto", display: "flex", justifyContent: "center" }}></i></button></> : <a href="/shop">Generate now</a>}</td>
                                                            </tr>
                                                            <tr>
                                                                <td><b>Production</b></td>
                                                                <td>{licenses.Production ? <>{licenses.Production} <button onClick={() => refreshLicense("production")} className='btn btn-primary' style={{ margin: "auto 10px" }}><i className='mdi mdi-refresh' style={{ margin: "auto", display: "flex", justifyContent: "center" }}></i></button></> : <a href="/shop">Generate now</a>}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
